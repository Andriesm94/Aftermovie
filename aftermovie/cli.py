...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Trim every clip in a folder to the beat length of a song and stitch them into one aftermovie."
    )
    ...
    parser.add_argument("--ms", type=int, help="Manually specify the exact clip length in milliseconds (overrides BPM/--song)")
    parser.add_argument("--beats-per-clip", type=int, default=1, help="How many beats each clip should span (default: 1)")
...
def main() -> None:
    ...
    if args.ms:
        ...
        beat_ms = beat_duration_ms(bpm, args.beats_per_clip)

    print(f"Clip length: {beat_ms} ms")
